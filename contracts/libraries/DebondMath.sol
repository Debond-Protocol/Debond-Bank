...
library DebondMath {
  ...
  function fixedInterestRate(
    ...
    uint256 floatingRate = floatingInterestRate(_fixRateBond, _floatingRateBond, _benchmarkIR);
   ...
   */
  function calculateInterestRate(uint256 _duration, uint256 _interestRate) public pure returns (uint256 interest) {
   ...
   */
  function lastMonthAverageLiquidityFlow(uint256 _sumOfLiquidityFlow, uint256 _benchmarkIR) public pure returns (uint256 averageFlow) {
  ...
  function floatingETA(
    ...
    int256 deficit = _deficitOfBond(_sumOfLiquidityFlow, _benchmarkIR, _sumOfLiquidityOfLastNonce);
  ...
  function _deficitOfBond(
    ...
    deficit = (int256(_sumOfLiquidityFlow) * (1 ether + int256(_benchmarkIR))) / 1 ether - int256(_sumOfLiquidityOfLastNonce);
  }
