...
abstract contract BankRouter {
  ...
  function swapExactTokensForTokens(
    ...
    uint256[] memory amounts = IAPM(apmAddress).getAmountsOut(_amountIn, _path);
  ...
  function swapExactEthForTokens(
    ...
    uint256[] memory amounts = IAPM(apmAddress).getAmountsOut(amountIn, _path);
  ...
  function getReserves(address _tokenA, address _tokenB) external view returns (uint256 _reserveA, uint256 _reserveB) {
  ...
  function _removeWETHLiquidity(uint256 _amount) internal {
  ...
  function _addLiquidityDbitPair(
    ...
    uint256 amountDBITToMint = _convertToDbit(_amount, _tokenAddress);
  ...
  function _addLiquidityDgovPair(
    ...
    uint256 amountDBITToMint = _tokenAddress == DBITAddress ? _amount : _convertToDbit(uint128(_amount), _tokenAddress);
    uint256 amountDGOVToMint = convertDbitToDgov(amountDBITToMint);
  ...
  function _addLiquidityDbitETHPair(uint256 _amount) internal {
    IWETH(WETHAddress).transfer(apmAddress, _amount);

    uint256 amountDBITToMint = _convertToDbit(_amount, WETHAddress);
    IDebondToken(DBITAddress).mintCollateralisedSupply(apmAddress, amountDBITToMint);

    _updateWhenAddLiquidity(_amount, amountDBITToMint, WETHAddress, DBITAddress);
  }

  function _addLiquidityDgovETHPair(uint256 _amount) internal {
    IWETH(WETHAddress).transfer(apmAddress, _amount);

    uint256 amountDBITToMint = _convertToDbit(uint128(_amount), WETHAddress);
    uint256 amountDGOVToMint = convertDbitToDgov(amountDBITToMint);
  ...
  function _addLiquidityDbitDgov(address _from, uint256 DBITamount) internal {
    IERC20(DBITAddress).safeTransferFrom(_from, apmAddress, DBITamount);

    uint256 amountDGOVToMint = convertDbitToDgov(DBITamount);
  ...
  function _swap(
    ...
    for (uint256 i; i < path.length - 1; i++) {
      ...
      IAPM(apmAddress).swap(amount0Out, amount1Out, input, output, to);
    }
  }
  ...
  function _convertToDgov(address _tokenAddress, uint256 _amount) internal view returns (uint256 amountDgov) {
    uint256 amountDBITToMint = _tokenAddress == DBITAddress ? _amount : _convertToDbit(uint128(_amount), _tokenAddress);
